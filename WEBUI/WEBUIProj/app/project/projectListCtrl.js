...
define(dependencies, function (app, projectFactory) {
    app.controller("projectListCtrl", function ($rootScope, $scope, $stateParams, $location, projectFactory, masterdataFactory, ngToast) {
        'use strict';
        NProgress.start();
        ...
        if ($scope.proj_id != "") { $scope.showheader = true } else { $scope.showheader = false };
        if ($scope.proj_id == "") { $scope.showhead = true } else { $scope.showhead = false };
